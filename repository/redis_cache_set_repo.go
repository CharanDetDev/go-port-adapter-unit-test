...
func (rc *redisCacheRepo) Set(newMackRedisCache model.MakeNewRedisCache) error {
	...
	if newMackRedisCache.Key != "" {
		...
		if err != nil {
			logg.Printlogger("SET Cache error", fmt.Sprintf("%v | %v | KEY =  %v", setKey, err, logg.GetCallerPathNameFileNameLineNumber()))
			return err
		}
